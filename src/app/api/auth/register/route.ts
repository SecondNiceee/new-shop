  ...
  try {
    ...
    if (candidate) {
      if (candidate._verified) {
        ...
        const url = `${process.env.PAYLOAD_PUBLIC_SERVER_URL}/verify?token=${candidate._verificationToken}`;
        await sendEmail({message : createEmail({mode : "verify", url, userEmail:candidate.email}).html, to:candidate.email })
        return NextResponse.json(
