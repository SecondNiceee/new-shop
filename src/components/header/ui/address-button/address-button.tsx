...
const AddressButton: FC<IAddressButton> = ({ className }) => {
  ...
  return (
    <div onClick={clickHandler} className={`cursor-pointer items-center space-x-2 min-w-0 ${className}`}>
      <MapPin size={24} className="text-black flex-shrink-0" />
      <div className='flex flex-col'>
        {fullAddress ? (
            <>
            <p className="text-sm font-medium truncate max-w-[200px] lg:max-w-xs text-gray-900">
                {fullAddress}
            </p>
