...
export function ContactWidget() {
  ...
  return (
    <div className="fixed flex flex-col bottom-12 right-12 z-50">
      {/* Expanded State */}
      {isOpen && (
        <div className="mb-4 bg-white rounded-2xl shadow-lg border border-gray-200 p-4 min-w-[200px]">
          ...
          <div className="space-y-2">
            {/* Telegram */}
            <a href={siteSettings?.socialLinks.telegram || ""} className="w-full cursor-pointer flex justify-start items-center h-10 px-3 hover:bg-blue-50 text-gray-700">
              <Send className="h-4 w-4 mr-3 text-blue-500" />
              Telegram
            </a>

            {/* WhatsApp */}
            <a href={siteSettings?.socialLinks.whatsApp || ""} className="w-full flex cursor-pointer justify-start items-center h-10 px-3 hover:bg-green-50 text-gray-700">
              <MessageSquare className="h-4 w-4 mr-3 text-green-500" />
              WhatsApp
            </a>

            {/* Email */}
            <a href={`mailto:${siteSettings?.socialLinks.email}`} className="w-full flex cursor-pointer justify-start items-center h-10 px-3 hover:bg-gray-50 text-gray-700">
